...
bool is_spot_blocked_or_redundant(
    int32_t row, 
    int32_t column, 
    Game& game, 
    GameMachine& machine, 
...
std::vector<Move*> expand_moves(
    ...
    if (distance_travelled > 1
        && !machine.machine.get().is_flying()
        && !machine.machine.get().is_pull()
        && game.terrain[row][column] == Terrain::Marsh)
    {
        return std::vector<Move*> {};
    }
...
std::vector<Move*> calculate_moves(Game& game, GameMachine& machine) {
    ...
    std::vector<Move*> all_moves = expand_moves(0, machine.row, machine.column, machine, game, visited);
